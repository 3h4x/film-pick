...
async function fetchCredits(tmdbId: number): Promise<{ director: string | null; writer: string | null; actors: string | null }> {
  const res = await fetch(`${TMDB_BASE}/movie/${tmdbId}?append_to_response=credits`, {
  ...
  const writer = data.credits?.crew?.filter((c: any) => ["Screenplay", "Writer", "Story"].includes(c.job))
    .map((c: any) => c.name).join(", ") || null;
  const actors = data.credits?.cast?.slice(0, 5).map((c: any) => c.name).join(", ") || null;
...
async function main() {
  ...
  console.log(`Found ${movies.length} movies with tmdb_id. ${DRY_RUN ? "(DRY RUN)" : ""}`);

  const update = db.prepare("UPDATE movies SET director = ?, writer = ?, actors = ? WHERE id = ?");
  let fixed = 0;
  let unchanged = 0;
  let failed = 0;
  ...
  for (let i = 0; i < movies.length; i++) {
    ...
    try {
      ...
      const changed = credits.director !== m.director || credits.writer !== m.writer || credits.actors !== m.actors;
      if (changed) {
        if (!DRY_RUN) {
          update.run(credits.director, credits.writer, credits.actors, m.id);
        }
        console.log(`[${i + 1}/${movies.length}] FIXED "${m.title}" (${m.year}): ${m.director} -> ${credits.director}`);
        fixed++;
      } else {
        unchanged++;
      }
