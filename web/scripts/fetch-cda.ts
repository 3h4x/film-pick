...
const CATEGORY_LABELS: Record<string, string> = {
  akcji: "Akcja", animowane: "Animowane", biograficzne: "Biograficzne",
  dokumentalne: "Dokumentalne", dramaty: "Dramaty", europejskie: "Europejskie",
  fantasy: "Fantasy", historyczne: "Historyczne", horror: "Horror",
  komedie: "Komedie", kryminalne: "Kryminalne", muzyczne: "Muzyczne",
  nagradzane: "Nagradzane", obyczajowe: "Obyczajowe", polskie: "Polskie",
  przygodowe: "Przygodowe", psychologiczne: "Psychologiczne", romanse: "Romanse",
  "sci-fi": "Sci-Fi", sensacyjne: "Sensacyjne", thrillery: "Thrillery",
  wojenne: "Wojenne", western: "Western",
};

const CATEGORIES = [
  "akcji", "animowane", "biograficzne", "dokumentalne", "dramaty",
  "europejskie", "fantasy", "historyczne", "horror", "komedie",
  "kryminalne", "muzyczne", "nagradzane", "obyczajowe", "polskie",
  "przygodowe", "psychologiczne", "romanse", "sci-fi", "sensacyjne",
  "thrillery", "wojenne", "western",
];
...
async function scrapeCategory(category: string): Promise<CdaMovie[]> {
  ...
  const posterRegex = /<img class="cover-img" title="([^"]*)"[^>]*src="([^"]*)"/g;
  let match;
  while ((match = posterRegex.exec(html)) !== null) {
    const imgTitle = decodeEntities(match[1]).trim();
    const posterUrl = match[2].startsWith("//") ? `https:${match[2]}` : match[2];
    posterMap.set(imgTitle, posterUrl);
  }

  // Extract title+url from kino-title links
  const titleRegex = /<a href="(https:\/\/www\.cda\.pl\/video\/[^"]*\/vfilm)" class="kino-title">([^<]*)<\/a>/g;
...
async function enrichFromTmdb(title: string, year: number | null): Promise<TmdbEnrichment | null> {
  ...
  return {
    ...
    tmdb_poster: match.poster_path ? `https://image.tmdb.org/t/p/w300${match.poster_path}` : null,
  };
}
...
async function main() {
  ...
  for (let i = 0; i < movies.length; i++) {
    ...
    if (tmdb) {
      insertStmt.run(
        tmdb.tmdb_id, movie.category, movie.title, movie.year,
        tmdb.genre, tmdb.rating, movie.poster_url, movie.title,
        movie.url, tmdb.description
      );
      enriched++;
    } else {
      const pseudoId = Math.abs(hashCode(movie.url));
      insertStmt.run(
        pseudoId, movie.category, movie.title, movie.year,
        null, 0, movie.poster_url, movie.title, movie.url, null
      );
    }

    // Rate limit
    if (i % 35 === 34) await new Promise(r => setTimeout(r, 1000));
    if ((i + 1) % 50 === 0) console.log(`  [cda] ${i + 1}/${movies.length} (${enriched} enriched)`);
  }
  ...
  console.log(`[cda] Saved ${movies.length} movies in ${catCounts.size} categories:`);
