...
export function getCdaLookup(): { byTmdbId: Map<number, string>; byTitle: Map<string, string> } {
  const db = getDb();
...
export function enrichWithCda(
  ...
  return results.map((r) => {
    const cdaUrl = cdaLookup.byTmdbId.get(r.tmdb_id) || cdaLookup.byTitle.get(r.title.toLowerCase());
