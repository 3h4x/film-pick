...
export async function actorEngine(ctx: EngineContext): Promise<RecommendationGroup[]> {
  ...
  const actorCounts = new Map<number, { name: string; count: number; avgRating: number }>();
