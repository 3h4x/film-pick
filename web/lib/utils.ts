export function cleanTitle(title: string): string {
  return title
    ...
    .replace(/\b(720p|1080p|2160p|4k|uhd|bluray|blu-ray|brrip|bdrip|webrip|web-dl|hdtv|dvdrip|xvid|divx|x264|x265|h264|h265|hevc|aac|ac3|dts|remux|proper|repack|maxspeed|torentz|torrentz|3xforum|fxg|noir|flixflux|kingdom|galaxyrg|yify|fgt|psig|yts|ev|evo|hdrip|cd[1-2]|dvd|blurayrip)\b/gi, " ")
    .replace(/\b(www|ro|com|net|org|pl|uk|co|osloskop|unseen|shoket|fxg|english|polish|multi|dual|subs)\b/gi, " ")
    .replace(/[\.\s_-]+/g, " ")
  ...
  if (!year) {
    ...
    if (bareYear) {
      ...
      if (y >= 1900 && y <= 2099) {
        ...
        const remaining = name.substring(bareYear.index! + bareYear[0].length).toLowerCase();
        const hasTag = /\b(720p|1080p|2160p|bluray|dvdrip|xvid|webrip|web-dl|hdtv|x264|x265|aac|ac3)\b/i.test(remaining);
