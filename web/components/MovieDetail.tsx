...
export default function MovieDetail({ movie, onClose, onUpdate, onMerge, onSearchTMDb, onPersonClick, allMovies }: MovieDetailProps) {
  ...
  const [director, setDirector] = useState<string | null>(movie.director || null);
  ...
  const [filePath, setFilePath] = useState<string | null>(movie.file_path || null);
  ...
  const [standardizeMsg, setStandardizeMsg] = useState<{ type: 'success' | 'error', text: string, code?: string } | null>(null);
  ...
  const [personRatings, setPersonRatings] = useState<Record<string, { avg_rating: number; movie_count: number }>>({});
  ...
  useEffect(() => {
    ...
    const allPeople = [
      ...(movie.director || "").split(",").map(s => s.trim()).filter(Boolean),
      ...(movie.writer || "").split(",").map(s => s.trim()).filter(Boolean),
    ...
    if (uniquePeople.length > 0) {
      ...
      fetch(`/api/person-ratings?${params}`)
        .then(r => r.json())
        .then((results: { name: string; avg_rating: number; movie_count: number }[]) => {
          const map: Record<string, { avg_rating: number; movie_count: number }> = {};
          for (const r of results) {
            if (r.movie_count >= 1) {
              if (!map[r.name] || r.movie_count > map[r.name].movie_count) {
                map[r.name] = { avg_rating: r.avg_rating, movie_count: r.movie_count };
              }
            }
          }
    ...
    fetch(`/api/movies/${movie.id}`)
      ...
      .then(data => {
        ...
        if (data.movie) {
          ...
          if (data.movie.description && !description) setDescription(data.movie.description);
  ...
  const handleMerge = async (targetId: number) => {
    if (!confirm("Are you sure you want to merge these movies? This one will be deleted and its data moved to the target.")) return;
    setIsMerging(true);
    try {
  ...
  const handleSubtitleUpload = async (file: File) => {
    console.log(`[Subtitles] Starting upload: ${file.name} (${file.size} bytes)`);
    ...
    try {
      ...
      if (data.ok) {
        ...
        setSubtitlesList(prev => [...prev, { name: data.fileName, path: data.path }]);
      } else {
    ...
    return (allMovies || [])
      ...
      .map(m => {
        ...
        if (mergeQuery) {
          ...
          if (m.pl_title?.toLowerCase().includes(mergeQuery.toLowerCase())) score += 10;
        } else {
          // Auto-match score
          if (cleanMTitle === cleanCurrentTitle) score += 20;
          if (cleanMPlTitle && cleanMPlTitle === cleanCurrentPlTitle) score += 20;
  ...
  const handleStandardize = async () => {
    ...
    try {
      ...
      if (data.ok) {
        setStandardizeMsg({ type: 'success', text: data.message || 'Path standardized!' });
        setFilePath(data.newPath);
        if (data.newTitle) {
          setMovieTitle(data.newTitle);
        }
        if (onUpdate) {
          onUpdate({ ...movie, file_path: data.newPath, title: data.newTitle || movie.title });
        }
  ...
  const handleRemoveMissing = async () => {
    if (!confirm("Are you sure you want to remove this entry? The file is missing or unmounted.")) return;
    setIsRemoving(true);
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 60000); // 60s timeout
  ...
  const handleDeleteFull = async () => {
    const confirmation = confirm(`⚠️ DANGER: Are you sure you want to delete "${movie.title}" from BOTH the database and YOUR DISK?\n\nThis will permanently delete the entire movie folder and all its contents.`);
    if (!confirmation) return;

    setIsDeletingDisk(true);
    try {
      const res = await fetch(`/api/movies/${movie.id}/full`, { method: 'DELETE' });
      const data = await res.json();
  ...
  const handleDeleteDiskOnly = async () => {
    const confirmation = confirm(`Delete "${movie.title}" folder from disk?\n\nYour rating and metadata will be kept in the database.`);
    if (!confirmation) return;

    setIsDeletingDisk(true);
    try {
      const res = await fetch(`/api/movies/${movie.id}/full?disk_only=1`, { method: 'DELETE' });
      const data = await res.json();

      if (data.ok) {
        setFilePath(null);
        if (onUpdate) {
          onUpdate({ ...movie, file_path: null, extra_files: null, video_metadata: null });
        }
      } else {
        alert(data.error || "Failed to delete from disk");
      }
      ...
      >
        <div className="absolute top-6 right-6 z-50 flex items-center gap-2">
          {filePath && (
            <div className="flex items-center gap-2 mr-2">
              <button
                ...
                title="Play Movie"
              >
                <span className="text-base group-hover:scale-110 transition-transform">{isPlaying ? '⏳' : '▶️'}</span>
                <span className="text-xs uppercase tracking-wider hidden sm:inline">Play</span>
              </button>
              <button
          ...
          <button
            ...
            className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
              isMenuOpen ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'bg-gray-800/80 text-gray-500 hover:text-white hover:bg-gray-800'
            }`}
        ...
        {isMenuOpen && (
          ...
          >
            <div className="p-2 space-y-1">
              {onSearchTMDb && (
                <button
                  onClick={() => { setIsMenuOpen(false); onSearchTMDb(movie.title, movie.id); }}
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Fix Metadata</span>
                    <span className="text-[9px] text-indigo-500/60 font-bold uppercase tracking-tight">Search TMDb for correct match</span>
                  </div>
                </button>
              )}
              ...
              {!isMergeMode && (
                <button
                  onClick={() => { setIsMenuOpen(false); setIsMergeMode(true); }}
                  className="w-full text-left px-4 py-3 hover:bg-indigo-500/10 rounded-xl transition-colors flex items-center gap-3 group"
                >
                  <span className="text-base group-hover:scale-110 transition-transform">🔗</span>
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Merge Duplicates</span>
                      {hasMatches && (
                        <span className="flex h-1.5 w-1.5 rounded-full bg-indigo-500 animate-pulse" />
                      )}
                    </div>
                    <span className="text-[9px] text-indigo-500/60 font-bold uppercase tracking-tight">Combine two entries into one</span>
                  </div>
                </button>
              )}
              ...
              {standardizeMsg?.code === 'FILE_NOT_FOUND' && (
                <button
                  onClick={() => { setIsMenuOpen(false); handleRemoveMissing(); }}
                  disabled={isRemoving}
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-red-400 uppercase tracking-wider">Remove Missing Entry</span>
                    <span className="text-[9px] text-red-500/60 font-bold uppercase tracking-tight">File not found, clean up library</span>
                  </div>
                </button>
              )}
              ...
              {filePath && (
                <button
                  onClick={() => { setIsMenuOpen(false); handleDeleteDiskOnly(); }}
                ...
                >
                  <span className="text-base group-hover:scale-110 transition-transform">📂</span>
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-orange-400 uppercase tracking-wider">Delete Files</span>
                    <span className="text-[9px] text-orange-500/60 font-bold uppercase tracking-tight">Remove from disk, keep in library</span>
                  </div>
                </button>
              )}
              ...
              {filePath && (
                <button
                  onClick={() => { setIsMenuOpen(false); handleDeleteFull(); }}
                  disabled={isDeletingDisk}
                  className="w-full text-left px-4 py-3 hover:bg-red-500/10 rounded-xl transition-colors flex items-center gap-3 group"
                >
                  <span className="text-base group-hover:scale-110 transition-transform">🔥</span>
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-red-400 uppercase tracking-wider">Remove from Library</span>
                    <span className="text-[9px] text-red-500/60 font-bold uppercase tracking-tight">Delete files and library entry</span>
                  </div>
                </button>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="relative aspect-[2/3]">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      (e.target as HTMLImageElement).parentElement?.classList.add('flex', 'items-center', 'justify-center', 'bg-gray-800');
            ...
            <div className="bg-gray-800/40 rounded-2xl p-5 border border-gray-700/30 space-y-4">
              {director && (
                <div className="space-y-1.5">
                  <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Director</p>
                    ...
                    {director.split(',').map((d, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-2">
                          <button onClick={() => onPersonClick?.(name)} className="text-white text-sm font-medium hover:text-indigo-400 transition-colors text-left">{name}</button>
                          {pr && (
              ...
              {writer && (
                <div className="space-y-1.5">
                  <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Scenario</p>
                  <div className="flex flex-col gap-1">
              ...
              {actors && (
                <div className="space-y-1.5">
                  <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Actors</p>
                  <div className="flex flex-col gap-1">
            ...
            {(videoMetadata || isLoadingMetadata) && (
              <div className="bg-gray-800/40 rounded-2xl p-5 border border-gray-700/30 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-indigo-400">Technical Details</h3>
                  {isLoadingMetadata && <span className="animate-pulse text-[10px] text-gray-500 font-bold uppercase">Loading...</span>}
                </div>
                ...
                {videoMetadata && !videoMetadata.error && (
                  <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                    <div className="space-y-1">
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter">Resolution</p>
                      <div className="text-sm text-gray-200 font-medium flex items-center gap-1.5 flex-wrap">
                        ...
                        {videoMetadata.video?.width >= 3840 ? (
                          <span className="px-1 py-0.5 bg-yellow-500/10 text-yellow-500 text-[9px] font-black rounded border border-yellow-500/20">4K</span>
                        ) : videoMetadata.video?.width >= 1920 ? (
                          <span className="px-1 py-0.5 bg-blue-500/10 text-blue-500 text-[9px] font-black rounded border border-blue-500/20">FHD</span>
                        ) : videoMetadata.video?.width >= 1280 ? (
                          <span className="px-1 py-0.5 bg-green-500/10 text-green-400 text-[9px] font-black rounded border border-green-500/20">HD</span>
                        ) : (
                          <span className="px-1 py-0.5 bg-gray-500/10 text-gray-400 text-[9px] font-black rounded border border-gray-500/20 uppercase">SD</span>
                        )}
                      </div>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter">Video Codec</p>
                      <p className="text-sm text-gray-200 font-medium uppercase">{videoMetadata.video?.codec}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter">File Size</p>
                      <p className="text-sm text-gray-200 font-medium">{(videoMetadata.size / (1024 * 1024 * 1024)).toFixed(2)} GB</p>
                    </div>
                    <div className="space-y-1">
                    ...
                    {videoMetadata.audio && videoMetadata.audio.length > 0 && (
                      <div className="col-span-2 space-y-2 pt-2 border-t border-gray-700/30">
                        ...
                        <div className="space-y-2">
                          {videoMetadata.audio.map((audio: any, idx: number) => (
                            <div key={idx} className="flex items-center justify-between bg-gray-900/40 px-2.5 py-1.5 rounded-lg border border-gray-700/20">
                              <span className="text-xs text-gray-300 font-medium uppercase">{audio.codec}</span>
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] text-gray-500 font-bold">{audio.channels} ch</span>
                                {audio.language && (
          ...
          <div className="lg:col-span-8 space-y-8">
            {/* Title & Actions */}
            <div className="space-y-1">
              <h2 className="text-3xl font-black text-white tracking-tight">{movieTitle}</h2>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {movie.source && (
                <span className="text-[10px] font-black px-2.5 py-1 bg-indigo-500/10 text-indigo-400 rounded-lg uppercase tracking-widest border border-indigo-500/20">
                  {movie.source === 'tmdb' ? 'TMDb' : movie.source.toUpperCase()}
                </span>
              )}
              {filePath && (
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-6">
                  {(userRating != null && userRating > 0) ? (
                    <div className="space-y-1.5">
                      <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">My Rating</p>
                  ...
                  {movie.rating != null && movie.rating > 0 && (
                    <div className="space-y-1.5">
                      <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Global</p>
                      <div className="flex items-center gap-3">
                  ...
                  {(!userRating || userRating === 0) && (
                    <div className="space-y-1.5 ml-2">
                      <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Rate this movie</p>
                      <div className="flex flex-col gap-1">
                ...
                <div className="space-y-4">
                  {movie.genre && (
                    <div className="space-y-2">
                      <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Genres</p>
                ...
                {isMergeMode && (
                  <div className="bg-gray-800 p-4 rounded-2xl border border-indigo-500/30 animate-in fade-in zoom-in-95 duration-200 shadow-xl">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-[10px] font-black uppercase text-indigo-400 tracking-widest">Merge target:</span>
                      <button onClick={() => setIsMergeMode(false)} className="text-gray-500 hover:text-white text-[10px] font-bold uppercase tracking-wider">cancel</button>
                    </div>
                    <input
                      autoFocus
                      type="text"
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                      {potentialMerges.map(m => (
                        ...
                        >
                          <span className="truncate">{m.title} <span className="text-gray-500 font-bold ml-1">[{m.year}]</span></span>
                          <span className="flex-shrink-0 opacity-0 group-hover:opacity-100 text-indigo-400 font-black uppercase text-[9px] ml-2 tracking-tighter">Merge →</span>
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <p className="text-[10px] text-indigo-400 font-black uppercase tracking-widest">Embedded Player</p>
                    {movie.extra_files && (
                      <div className="flex items-center gap-1 bg-gray-800 rounded-lg p-0.5">
                        {[filePath, ...JSON.parse(movie.extra_files)].map((_, i) => (
                          <button
                            key={i}
                            onClick={() => setActivePart(i)}
                            className={`px-2 py-0.5 text-[9px] font-bold rounded ${
                              activePart === i ? 'bg-indigo-500 text-white' : 'text-gray-500 hover:text-gray-300'
                            }`}
                          >
                            Part {i + 1}
                          </button>
                ...
                <p className="text-[10px] text-gray-500 italic text-center">
                  Note: MKV support varies by browser. Use "Play in VLC" for best compatibility.
                </p>
              </div>
            ) : description && (
              <div className="space-y-2">
                <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Plot Summary</p>
                <p className="text-gray-300 text-base leading-relaxed font-normal">{description}</p>
              </div>
            )}
                      ...
                      >
                        <div>
                          <p className="text-xs font-bold text-gray-200 group-hover:text-indigo-300 transition-colors">
                            {m.title} {m.year && <span className="text-gray-500">({m.year})</span>}
                          </p>
                          <p className="text-[10px] text-gray-500 font-medium">
                            {m.source} • {m.file_path ? 'Local' : 'Remote'}
                          </p>
                        </div>
                        <span className="text-indigo-500/50 group-hover:text-indigo-500 transition-colors">→</span>
                      </button>
                    ))
                  ) : (
                    <p className="text-center py-4 text-gray-500 text-xs italic">
                      {mergeQuery ? "No matching movies found." : "Search or select from suggestions below..."}
                    </p>
                  )}
                </div>
            ...
            <div className="space-y-3">
              <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Subtitle Management</p>
              <div className="bg-gray-800/30 rounded-2xl p-5 border border-gray-700/30">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <span className="text-lg">⌨️</span>
                    <span className="text-xs font-bold text-gray-300 uppercase tracking-widest">Available Tracks</span>
                  </div>
                  {hasSubtitles && (
                ...
                {filePath ? (
                  <div className="space-y-4">
                    {subtitlesList.length > 0 ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {subtitlesList.map((sub, i) => (
                          <div key={i} className="flex items-center gap-3 bg-gray-900/60 px-3 py-2 rounded-xl border border-gray-700/20 group">
                            <span className="text-indigo-500 font-black text-[9px] tracking-tighter">POL</span>
                            <span className="text-xs text-gray-400 truncate flex-1">{sub.name}</span>
                            <span className="text-[10px] opacity-0 group-hover:opacity-100 transition-opacity text-gray-600">.srt</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="text-center py-6 bg-gray-900/40 rounded-2xl border border-dashed border-gray-700/50">
                        <p className="text-gray-500 text-xs font-medium">No localized subtitles found in movie folder.</p>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                      ...
                      <label 
                        ...
                        onDrop={onDropSub}
                      >
                        <span className="text-lg">{isSubtitleUploading ? '⏳' : isDraggingSub ? '✨' : '📁'}</span>
                  ...
                  <div className="text-center py-8">
                    <p className="text-gray-500 text-sm mb-4 italic">Metadata only. Link a local file to manage subtitles.</p>
                    <a
                      href={`https://www.opensubtitles.org/en/search2/moviename-${encodeURIComponent(movie.title)}/sublanguageid-pol`}
                      target="_blank"
                      rel="noopener noreferrer"
            ...
            {filePath && (
              <div className="space-y-3">
                <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Storage & File System</p>
                <div className="bg-gray-800/40 rounded-2xl p-5 border border-gray-700/30 space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-2 flex-1 min-w-0">
                      ...
                      <p className="text-gray-300 text-xs font-mono break-all bg-gray-900/40 px-3 py-2 rounded-lg border border-gray-700/20 flex-1 leading-relaxed">
                        {filePath}
                        {movie.extra_files && JSON.parse(movie.extra_files).length > 0 && (
                          <span className="ml-2 px-1 py-0.5 bg-indigo-500/20 text-indigo-400 text-[9px] font-black rounded uppercase">Part 1</span>
                        )}
                      </p>
                    </div>
                    <div className="shrink-0 flex justify-end">
                      {(isStandard || isStandardNoYear) ? (
                        <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-green-500/10 border border-green-500/20">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest text-green-400">Standard</span>
                        </div>
                      ) : (
                        <button
                        ...
                        >
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest">Standardize</span>
                        </button>
                      )}
                    </div>
                  </div>

                  {movie.extra_files && JSON.parse(movie.extra_files).map((extraPath: string, idx: number) => (
                    <div key={idx} className="flex items-start gap-2 pl-4 border-l-2 border-gray-700/50">
                      <div className="p-2 bg-gray-900/60 rounded-lg shrink-0">
                      ...
                      <p className="text-gray-400 text-[10px] font-mono break-all bg-gray-900/20 px-3 py-2 rounded-lg border border-gray-700/10 flex-1 leading-relaxed">
                        {extraPath}
                        <span className="ml-2 px-1 py-0.5 bg-indigo-500/10 text-indigo-500/60 text-[9px] font-black rounded uppercase">Part {idx + 2}</span>
                      </p>
                    </div>
                  ))}
                </div>
                  ...
                  {standardizeMsg && (
                    <div className={`p-4 rounded-xl border animate-in fade-in slide-in-from-top-2 duration-300 ${
                      ...
                      <div className="flex items-start gap-3">
                        <span className="text-sm mt-0.5">{standardizeMsg.type === 'success' ? '✨' : '⚠️'}</span>
                        <div className="flex-1">
                          <p className="text-[10px] font-black uppercase tracking-widest mb-1">
                            {standardizeMsg.type === 'success' ? 'Library Synced' : 'Action Required'}
                          </p>
                          <p className="text-xs font-medium leading-relaxed">{standardizeMsg.text}</p>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <div className="flex items-center gap-6 pt-4 border-t border-gray-800">
              {movie.created_at && (
                <div className="space-y-1">
                  <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest">Added to Library</p>
                  <p className="text-[11px] text-gray-500 font-medium">{new Date(movie.created_at).toLocaleDateString()}</p>
                </div>
              )}
              {movie.rated_at && (
                <div className="space-y-1">
                  <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest">Last Rated</p>
                  <p className="text-[11px] text-gray-500 font-medium">{movie.rated_at}</p>
                </div>
              )}
            </div>
          </div>
