  ...
  return (
    <div className="max-w-2xl space-y-8">
      ...
      <section>
        ...
        <p className="text-gray-500 text-xs mb-3">Recommendations with these genres will be hidden</p>
      ...
      <section>
        ...
        <p className="text-gray-500 text-xs mb-3">Only recommend movies released after this year</p>
        <div className="flex items-center gap-3">
          <input
            ...
            onChange={(e) => update({ min_year: e.target.value ? parseInt(e.target.value, 10) : null })}
      ...
      <section>
        ...
        <p className="text-gray-500 text-xs mb-3">Filter out recommendations below this rating</p>
        <div className="flex items-center gap-3">
          <input
            type="number"
            min={0}
            ...
            onChange={(e) => update({ min_rating: e.target.value ? parseFloat(e.target.value) : null })}
            className="bg-gray-800/60 border border-gray-700/30 rounded-lg px-3 py-2 text-white text-sm w-32 focus:outline-none focus:border-indigo-500/50"
          />
          <div className="flex gap-1">
            {[6, 6.5, 7, 7.5, 8].map((r) => (
      ...
      <section>
        ...
        <p className="text-gray-500 text-xs mb-3">Maximum number of recommendations in each group</p>
        <div className="flex gap-1">
          {[5, 10, 15, 20, 30].map((n) => (
            <button
              key={n}
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        {dirty && <span className="text-yellow-500/70 text-xs">Unsaved changes</span>}
      </div>
    </div>
  );
}
