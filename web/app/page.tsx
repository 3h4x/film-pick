...
export default function Home() {
  ...
  const [recGroups, setRecGroups] = useState<Record<string, RecommendationGroup[]>>({});
  ...
  const recsLoading = recCategory === "all"
    ? Object.values(recLoading).some(Boolean) || (!hasAnyRecs && activeTab === "recommendations" && movies.length > 0)
    : (recLoading[recCategory] ?? false) || (!recGroups[recCategory] && activeTab === "recommendations" && movies.length > 0);
  ...
  const fetchRecsForCategory = useCallback(async (category: string, refresh = false) => {
    if (category === "all") {
      // Fetch each engine that isn't cached yet (or all if refresh)
      const engineKeys = Object.keys(REC_CATEGORIES.slice(1).map((c) => c.value));
        ...
        next[key] = groups
          .map((g) => ({
            ...
            recommendations: g.recommendations.filter((r: any) =>
              r.tmdb_id !== tmdbId && (!title || r.title !== title)
            ),
          }))
          .filter((g) => g.recommendations.length > 0);
      }
  ...
  async function handleRecAction(tmdbId: number, action: string, rec: any) {
    ...
    if (action !== "dismiss") {
      const userRating = action === "liked" ? 8 : action === "disliked" ? 3 : action === "wishlist" ? null : 5;
          ...
          <>
            {initialLoad ? (
              <div className="animate-pulse space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
                  {Array.from({ length: 12 }).map((_, i) => (
                    <div key={i} className="rounded-xl overflow-hidden bg-gray-800/40 border border-gray-700/20">
              ...
              <div className="text-center py-24">
                ...
                <p className="text-gray-400 text-lg font-medium">Your library is empty</p>
                <p className="text-gray-600 text-sm mt-2">Import a folder or search to start building your collection</p>
          ...
          <>
            {movies.length === 0 ? (
              <div className="text-center py-24">
                ...
                <p className="text-gray-600 text-sm mt-2">Add some movies to your library first</p>
              </div>
            ) : recsLoading ? (
          ...
          <>
            {wishlistMovies.length === 0 ? (
              <div className="text-center py-24">
                ...
                <p className="text-gray-600 text-sm mt-2">Browse recommendations and bookmark films you want to watch</p>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
