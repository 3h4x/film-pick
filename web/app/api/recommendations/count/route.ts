import { getDb, getDismissedIds, getCachedEngine, getRecommendedMovies } from "@/lib/db";
import { engines } from "@/lib/engines";

export async function GET() {
  const db = getDb();
  const dismissedIds = getDismissedIds(db);
  const movieCount = db.prepare("SELECT COUNT(*) as c FROM movies").get() as { c: number };
  let total = 0;
    ...
    if (def.dbBacked) {
      ...
      if (cached) {
        for (const group of cached as any[]) {
          total += (group.recommendations || []).filter((r: any) => !dismissedIds.has(r.tmdb_id)).length;
        }
      }
    }
  }
