...
export async function POST() {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const newFiles = allFiles.filter(f => !getMovieByFilePath(db, f.filePath));
      ...
      for (let i = 0; i < newFiles.length; i++) {
        ...
        if (existingByTitle) {
          db.prepare("UPDATE movies SET file_path = ? WHERE id = ?").run(file.filePath, existingByTitle.id);
          linked++;
          continue;
        }
      ...
      sendUpdate({ type: "complete", added, linked, removed, unchanged, failed, total: allFiles.length });
      controller.close();
    },
  });
