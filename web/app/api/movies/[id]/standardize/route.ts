...
export async function POST(
  ...
  const mergeMovies = (sourceId: number, targetId: number) => {
    const s = db.prepare("SELECT * FROM movies WHERE id = ?").get(sourceId) as any;
    const t = db.prepare("SELECT * FROM movies WHERE id = ?").get(targetId) as any;
    if (!s || !t) return;
    ...
    if (s.user_rating || t.user_rating) {
      updates.user_rating = Math.max(Number(s.user_rating || 0), Number(t.user_rating || 0)) || null;
    }

    // 2. rating: take max
    if (s.rating || t.rating) {
      updates.rating = Math.max(Number(s.rating || 0), Number(t.rating || 0)) || null;
    }
  ...
  if (!movie.file_path) {
    return Response.json({ error: "No file path associated with this movie" }, { status: 400 });
  }
    ...
    if (fsSync.existsSync(newPath)) {
      ...
      if (finalTitle !== movie.title || finalYear !== movie.year || true) { // Always check for conflicts with the final title
          ...
          if (conflict) {
              ...
              return Response.json({ ok: true, message: "Movies merged and DB updated", newPath, newTitle: finalTitle, newYear: finalYear });
          }
      }

      db.prepare("UPDATE movies SET file_path = ?, title = ?, year = ? WHERE id = ?").run(newPath, finalTitle, finalYear, movieId);
      return Response.json({ ok: true, message: "DB updated (file was already moved)", newPath, newTitle: finalTitle, newYear: finalYear });
    }

    // If the file is missing, we check for a 'delete_missing' query param to cleanup DB
    const deleteMissing = _request.nextUrl.searchParams.get("delete_missing") === "true";
    if (deleteMissing) {
  ...
  const conflict = db.prepare("SELECT id, file_path FROM movies WHERE title = ? AND year = ? AND id != ? AND type = 'movie'").get(finalTitle, finalYear, movieId) as any;
  if (conflict) {
    console.log(`- Merge conflict detected during standardization (ID=${conflict.id})`);
    ...
    if (!conflict.file_path) {
      ...
      if (!fsSync.existsSync(conflict.file_path)) {
         console.log(`- Conflicting file not found, removing placeholder: ${conflict.file_path}`);
         db.prepare("DELETE FROM movies WHERE id = ?").run(conflict.id);
      } else {
  ...
  if (normOld === normNew && oldTitle === finalTitle && movie.year === finalYear) {
    return Response.json({ ok: true, message: "Path and title are already standard", path: newPath });
  }
  ...
  try {
    ...
    if (normOld !== normNew) {
      ...
      for (const file of filesInOldDir) {
        ...
        if (subtitleExts.includes(fileExt) && (fileNameNoExt === oldFileNameNoExt || fileNameNoExt.startsWith(oldFileNameNoExt))) {
          ...
          const newSubName = fileNameNoExt.replace(oldFileNameNoExt, finalTitle) + targetSubExt;
    ...
    db.prepare("UPDATE movies SET file_path = ?, title = ?, year = ?, extra_files = ? WHERE id = ?").run(newPath, finalTitle, finalYear, updatedExtraFiles, movieId);
    ...
    try {
      if (normOld !== normNew) {
        ...
        if (oldDir !== libraryRoot && oldDir !== targetDir) {
           ...
           const junkFiles = new Set([".DS_Store", ".AppleDouble", "._.DS_Store", "Thumbs.db", "desktop.ini"]);
           const meaningful = files.filter(f => !f.startsWith("._") && !junkFiles.has(f));
      ...
      console.warn(`- Failed to cleanup old directory: ${e instanceof Error ? e.message : String(e)}`);
    }
    ...
    return Response.json({ error: error.message || "Failed to standardize path" }, { status: 500 });
  }
}
