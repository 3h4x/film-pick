...
describe("sync: insertMovie deduplication", () => {
  ...
  it("links file_path to existing movie with same title+year when file_path is null", () => {
    ...
    const filePath =
      "/Volumes/video/Movies/The Shining (1980)/The.Shining.1980.mkv";
    insertMovie(db, {
      title: "The Shining",
      year: 1980,
      genre: "Horror",
  ...
  it("updates file_path on existing movie with same title+year even if it already has a different file_path", () => {
    ...
    const newPath =
      "/Volumes/video/Movies/The Matrix (1999)/The.Matrix.1999.1080p.mkv";
    insertMovie(db, {
      title: "The Matrix",
      year: 1999,
      genre: "Sci-Fi",
...
describe("sync: parseFilename for sync matching", () => {
  ...
  it("handles special characters in title", () => {
    const result = parseFilename(
      "Dalí & Disney A Date with Destino [2010].avi",
    );
